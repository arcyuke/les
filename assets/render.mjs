...
function media(src, alt, options = {}) {
  const {className = '', label = 'Место для фотографии', index = '01', eager = false, tone = 'sun'} = options;
  ...
  return `<div class="media-frame media-placeholder tone-${tone} ${className}" role="img" aria-label="${esc(alt)}"><span class="media-number">${esc(index)}</span><span class="media-plus" aria-hidden="true">+</span><div class="media-brand" aria-hidden="true">ЛЕС</div><p>${esc(label)}</p><i aria-hidden="true"></i></div>`;
}
...
function programCard(program, index, content) {
  const picture = program.image ? media(program.image, program.imageAlt, {className:'program-media',label:program.tag,index:number(index)}) : '';
  return `<article class="program-card reveal" data-category="${esc(program.category)}" style="--delay:${(index % 3) * 70}ms">${picture}<div class="program-top"><span class="program-number">${number(index)}</span><span class="pill">${esc(program.category)}</span></div><div class="program-copy"><p class="program-tag">${esc(program.tag)}</p><h3 id="program-${index}">${esc(program.title)}</h3><p>${esc(programDescription(program))}</p></div><div class="program-bottom"><div><small>${esc(content.ui.price)}</small><strong>${esc(program.price || content.ui.priceUnknown)}</strong>${program.unit ? `<span>${esc(program.unit)}</span>` : ''}</div>${textLink(`booking.html?program=${encodeURIComponent(program.id || program.title)}`, content.ui.book)}</div></article>`;
}

function venueCard(venue, index, content, large = false) {
  return `<article class="venue-card ${large ? 'venue-card-large' : ''} reveal" style="--delay:${index * 90}ms">${media(venue.image, venue.imageAlt, {className:'venue-media',label:'Фотография площадки',index:number(index),tone:index ? 'leaf' : 'sun'})}<div class="venue-copy"><div class="venue-meta"><span>${number(index)}</span><span>${esc(venue.district)}</span></div><h3>${esc(venue.name)}</h3><p>${esc(large ? venue.description : venue.address)}</p>${large ? `<p class="venue-address">${esc(venue.address)}</p>${button(safeUrl(venue.mapUrl), content.ui.map, 'line', true)}` : textLink('venues.html', content.ui.details)}</div></article>`;
}

function pageHero(page, index, mediaLabel = '') {
  return `<section class="page-hero"><div class="page-hero-copy hero-enter"><p class="kicker">${esc(page.eyebrow)}</p><h1>${esc(page.title)} <em>${esc(page.titleAccent)}</em></h1><p class="page-lead">${esc(page.text)}</p></div><div class="page-hero-mark hero-enter" style="--delay:120ms"><span>${index}</span><p>${esc(mediaLabel || 'ЛЕС · Новосибирск')}</p></div></section>`;
}
