...
test('all page and asset links exist; all public pages contain both phones and privacy',async()=>{for(const page of pages){const html=await readFile(path.join(root,filenames[page]),'utf8');assert.ok(html.includes('+7 (913) 792-56-26'));assert.ok(html.includes('+7 (983) 005-38-68'));assert.ok(html.includes('privacy.html'));assert.equal((html.match(/<h1[ >]/g)||[]).length,1);for(const match of html.matchAll(/(?:src|href)="([^"#]+)"/g)){const url=match[1].split('?')[0].split('#')[0];if(!/^(https?:|tel:|data:|blob:)/.test(url))await access(path.join(root,url));}}});
